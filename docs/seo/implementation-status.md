...
- `app/sitemap.ts` (segments via `generateSitemaps()`; ids in `lib/seo/sitemapSegments.ts`), `app/robots.ts`, `app/openapi.json/route.ts`.
...
- This baseline does **not** include a full crawl of every sitemap segment or the 39 known-404 URLs from the handoff appendix — that reconciliation is DEV-01 acceptance work, not DEV-00.
